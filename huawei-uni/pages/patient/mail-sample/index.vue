  ...
  <view class="page-container">
    ...
    <view class="form-card">
      <view class="form-item">
        <text class="form-label">检测扣次方式</text>
        <view class="mode-row">
          <view class="mode-item" :class="{ active: !usePackage }" @click="selectSingle">单次检测</view>
          <view class="mode-item" :class="{ active: usePackage }" @click="selectPackageMode">套餐扣次</view>
        </view>
        <text v-if="packageDeducted" class="deducted-tip">该管码已扣次，本次重复提交不会再次扣除</text>
      </view>
      <view v-if="usePackage" class="form-item">
        <text class="form-label required">选择患者套餐</text>
        <picker :disabled="packageDeducted" :range="packages" range-key="display_name" @change="onPackageChange">
          <view class="form-input picker-value" :class="{ placeholder: !form.sale_order_id }">{{ selectedPackageName }}</view>
        </picker>
      </view>
...
export default {
  data() {
    return {
      ...
      form: { sample_code: '', sale_order_id: 0, sender_name: '', sender_phone: '', sender_address: '', express_type: 'auto', express_company: '京东物流', tracking_no: '', notes: '' }
    }
  },
  computed: {
    selectedExpressCompany() { return this.expressCompanies[this.expressCompanyIndex] || this.expressCompanies[0] },
    selectedPackageName() {
      const item = this.packages.find(pkg => Number(pkg.order_id) === Number(this.form.sale_order_id))
      return item ? item.display_name : '请选择要扣除次数的套餐'
    }
  },
  onLoad(options) {
    ...
    else this.loadPackages()
  },
  methods: {
    async loadPackages() {
      try {
        const res = await uniAPI.getMyPackages(this.form.sample_code)
        const data = res.data || {}
        const selectedOrderId = Number(data.selected_order_id) || 0
        this.packageDeducted = Boolean(data.package_deducted)
        this.packages = (data.list || []).filter(item => Number(item.remaining_count) > 0 || Number(item.order_id) === selectedOrderId).map(item => ({
          ...item,
          display_name: `${item.package_name}（剩余${item.remaining_count}/${item.detection_count}次）`
        }))
        if (selectedOrderId) {
          this.form.sale_order_id = selectedOrderId
    ...
    selectSingle() {
      if (this.packageDeducted) return
      this.usePackage = false
      this.form.sale_order_id = 0
    },
    selectPackageMode() {
      if (this.packageDeducted) return
      if (!this.packages.length) return uni.showToast({ title: '暂无可用套餐次数', icon: 'none' })
      this.usePackage = true
    },
    onPackageChange(event) {
      const item = this.packages[Number(event.detail.value)]
      this.form.sale_order_id = item ? Number(item.order_id) : 0
    },
    ...
    validateForm() {
      ...
      if (this.usePackage && !this.form.sale_order_id) return '请选择要扣除次数的套餐'
    ...
    async submitForm() {
      ...
      try {
        ...
        const data = res.data || {}
        const title = data.detection_number ? `已扣第${data.detection_number}次，剩余${data.remaining_count}次` : '提交成功'
...
.picker-value.placeholder { color: #9ca3af; }.mode-row { display: flex; gap: 16rpx; }.mode-item { flex: 1; padding: 20rpx; border-radius: 12rpx; background: #f3f6fa; color: #606f7b; font-size: 25rpx; text-align: center; }.mode-item.active { background: #1677ff; color: #fff; }.deducted-tip { display: block; margin-top: 12rpx; color: #389e0d; font-size: 22rpx; }
