    ...
    <view v-else class="detail-container">
      <view class="report-header">
        <view class="header-top">
          <text class="report-title">检验报告</text>
      ...
      <view class="section" v-if="trendPoints.length > 1">
        ...
        <canvas canvas-id="employeeTrendChart" id="employeeTrendChart" class="trend-chart"></canvas>
      ...
      <view class="section">
        ...
        <view class="info-list">
          ...
          <view class="info-row">
            <text class="lbl">生成人员</text>
            <text class="val">{{ report.generated_by || '-' }}</text>
          </view>
          <view class="info-row">
            <text class="lbl">审核者</text>
            <text class="val">{{ report.reviewer || report.reviewed_by || '-' }}</text>
          </view>
          <view class="info-row" v-if="report.reviewed_time">
            <text class="lbl">审核时间</text>
            <text class="val">{{ report.reviewed_time }}</text>
          </view>
          <view class="info-row" v-if="report.rejected_reason">
            <text class="lbl">拒绝原因</text>
            <text class="val">{{ report.rejected_reason }}</text>
          </view>
        </view>
      </view>

      <view v-if="report.status === 'pending'" class="action-bar">
        <button class="action-btn reject" @click="rejectReport">拒绝</button>
        <button class="action-btn approve" @click="approveReport">审核通过</button>
      </view>
...
export default {
  ...
  methods: {
        ...
        success: async (res) => {
          if (res.confirm) {
            try {
              ...
              const response = await uniAPI.reviewReport(this.report.id, payload)
              if (response.success) {
      ...
      uni.showModal({
        ...
        success: async (res) => {
          if (res.confirm) {
            ...
            try {
              const response = await uniAPI.reviewReport(this.report.id, { status: 'rejected', rejectedReason: reason })
              if (response.success) {
    ...
    drawTrendChart() {
      ...
      const ctx = uni.createCanvasContext('employeeTrendChart', this)
      const width = 320
      const height = 150
      const padding = 28
...
.trend-row { display: grid; grid-template-columns: 1.2fr 0.8fr 0.5fr 1.2fr; gap: 10rpx; align-items: center; padding: 18rpx 0; border-bottom: 1rpx solid #eef1f5; }
